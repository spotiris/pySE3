'''
Provides convention - consistent operations and conversions between quaternions, Euler angles and rotation matricies.
Conventions:
 * Right-handed system
 * z-y'-x'' (intrinsic) rotations for Euler Angles (specified in x,y,z order). Also equivalent to Z-Y-X extrinsic rotation.
 * Quaternions are in [xw,x,y,z] order.
 * Both quaternions and rotation matrices are right-multiply (y' = R * y; q' = q * V, where V is homogeneous R4 vector)
...
class EulerAngles(object):
    ...
    def from_rotation_matrix(rotation_matrix, rotation_order='ZYX'):
        ...
        if rotation_order == 'ZYX':
            e = np.array([
                [atan2(rotation_matrix[1, 2], rotation_matrix[2, 2])],
    ...
    def from_quaternion(q, rotation_order='ZYX'):
        ...
        if rotation_order == 'ZYX':
            e = np.array([
                [atan2(2 * q[0] * q[1] + 2 * q[2] * q[3], q[0] * q[0] - q[1] * q[1] - q[2] * q[2] + q[3] * q[3])],
                [asin(2 * q[0] * q[2] - 2 * q[1] * q[3])],
                [atan2(2 * q[0] * q[3] + 2 * q[1] * q[2], q[0] * q[0] + q[1] * q[1] - q[2] * q[2] - q[3] * q[3])]],
                dtype=np.float64)
        else:
...
class Quaternion(object):
    ...
    def multiply(q, r):
        return Rotation(np.array([
            ...
            [q[0] * r[3] + q[1] * r[2] - q[2] * r[1] + q[3] * r[0]]], dtype=np.float64).reshape((4, 1)),
